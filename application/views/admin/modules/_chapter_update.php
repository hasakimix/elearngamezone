...
<div class="container">
    <div class="d-flex justify-content-between mb-3">
        <h5>Update  <span class="text-info"><?= $module["module_topic"] ?></span> - Chapter</h5>
    </div>

    <form action="<?= base_url("admin/modules/chapter_save/update") ?>" method="POST" id="save-chapter-form">
        <input type="hidden" id="module_id" name="module_id" value="<?= $module["module_id"] ?>">
        <input type="hidden" id="chapter_id" name="chapter_id" value="<?= $chapter["chapter_id"] ?>">
        <div class="form-group">
            <label for="library-name">Chapter Name:</label>
            <input type="text" id="chapter_name" name="chapter_name" value="<?= $chapter["chapter_name"] ?>" placeholder="Enter Chapter name">
        </div>
        <div class="form-group">
            <label for="preview">Contents:</label>
            <textarea id="contents" name="contents" placeholder="Write a chapter contents..." rows="4">
                <?= base64_decode($chapter["contents"]) ?>
            </textarea>
        </div>
        <div class="d-flex">
